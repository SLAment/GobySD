...
# # Output
# bigpicturefile <- "/Users/lorena/Library/CloudStorage/Dropbox/VRwork/Analyses/7_Goby/IFB/06_Map2Reference/results/PomflaTH1xy-vs-fGobFla1_Dotplot.png"
# chr16plotfile <- "/Users/lorena/Library/CloudStorage/Dropbox/VRwork/Analyses/7_Goby/IFB/06_Map2Reference/results/PomflaTH1xy-vs-fGobFla1_Dotplot_chr16.png"
# chr16barfile <- "/Users/lorena/Library/CloudStorage/Dropbox/VRwork/Analyses/7_Goby/IFB/06_Map2Reference/results/PomflaTH1xy-vs-fGobFla1_chr16_bar.pdf"
...
# ============================
### Make a plot of the chromosome
# ============================

chr16SD <- chr16 %>% filter(QUERY == "ptg000042l", S1 > 29000000)

# Get total reference and query lengths (they're repeated in each row)
ref_len <- unique(chr16SD$COV_R)
query_len <- unique(chr16SD$COV_Q)

tick_interval <- 5e6
ticks <- seq(tick_interval, ref_len, by = tick_interval)

chr16bar <- ggplot(chr16SD) +
  # Background gray bar
  geom_rect(aes(xmin = 0, xmax = ref_len, ymin = 0.9, ymax = 1),
            fill = "gray90", color = "black") +
  
  # Aligned regions
  geom_rect(aes(xmin = S1, xmax = E1, ymin = 0.9, ymax = 1),
            fill = "gray30", alpha = 0.8) +
  
  # Tick marks just below the bar
  geom_segment(data = data.frame(x = ticks),
               aes(x = x, xend = x, y = 0.89, yend = 0.9),
               color = "black", size = 0.5) +
  
  # Labels for tick marks
  geom_text(data = data.frame(x = ticks),
            aes(x = x, y = 0.88, label = paste0(x / 1e6, " Mb")),
            size = 3, hjust = 0.5) +
  
  theme_minimal() +
  labs(x = NULL, y = NULL) +
  scale_x_continuous(expand = c(0, 0), limits = c(0, ref_len)) +
  theme(
...
ggsave(plot = chr16bar, 
       filename = chr16barfile,
       width = 10, height = 1.3)
...
(bigpicture <- dotplotter2(bigQcoords, minLENQ = 30000, linesize = 1.1) + 
  theme(axis.text.x = element_blank(),
        # strip.text.x = element_blank(),
        strip.text.y = element_blank(),
        panel.grid.major = element_line(color = "gray95")
        # panel.grid.major = element_blank() # commented: not pretty but it helps to understand the plot
        ) + 
